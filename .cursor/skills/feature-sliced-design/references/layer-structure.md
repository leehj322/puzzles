...
> **Naming convention:** FSD page slices export their root component as
> `*View`, not `*Page`, to avoid collisions with framework-level routing
> concepts (e.g., Next.js `page.tsx`). The framework's route file is a thin
> adapter that returns the View. See
> [`framework-integration.md`](./framework-integration.md) for the Next.js
> wiring example.
...
pages/
  home/
    ui/
      HomeView.tsx          ← Root component (note: View, not Page)
      HeroSection.tsx
      FeaturesGrid.tsx
    model/
...
```typescript
// pages/product-detail/ui/ProductDetailView.tsx
